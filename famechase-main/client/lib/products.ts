...
📊 परफॉर्ेंस मेट्रिक्स:
• औसत पहुंच: [आपकी पहु���च दर्ज करें]
...
• आपके प्रोडक्ट्स मेर��� ऑड���यंस के साथ perfectly align करते हैं
...
function generateContentCalendarContent(
  ...
  if (language === "hindi") {
    return `📅 3-मही��े कंे���ट कैलेंडर टेम्प्लेट
...
• हैशटैग और की���रड स्ट्रैटेजी रिफाइन करें
...
• भारतीय त्योहारों और ीजन का फायदा उ��ाएं
...
• कैरो��ेल पोस्ट (10 slides): ₹${rates.post} + 20%
...
• एक्सक्लूसिव���टी बढ़ाने पर अतिरिक्त शुल्क लागू
...
1. स्कोप
• अभियान उद्देश्यों, मैट्रिक्स और ���ंटेंट फॉर्मैट्स का स्पष्ट उल्लेख

2. भुगतान/बार्टर
• Paid: ₹XX,XXX + कर
• Barter: उत्पाद मूल्य ₹X,XXX; डिलीवरी के 3 दिनों के भीतर
...
${userNiche} क्रिएटर्स के लिए वायरल ऑडियो स्��्रैटेजी
...
• Text animations को rhythm क�� साथ
...
function generateAlgorithmSecretsContent(
  ...
  if (language === "hindi") {
    return `📱 इंस्टाग्राम एल्गोरिदम ऑप्टिमाइजेशन सीक्र��ट्स
...
मुझे लग��ा है कि हम एक amazing partnership create कर सकते हैं जो आप���े brand को मेरी highly engaged audience तक पहुंचाए।
...
मेरे engagement rates और audience quality को considering करते हुए, क्या हम rate को slightly adjust क��� सकते हैं?
...
${userNiche} क्िएटर्स के लिए ऑर्गेनिक रीच ��र एंगेजमेंट मास्टरी
...
3️⃣ Share valuable content only
4���⃣ Help others in the community
5️⃣ Use search before asking duplicate questions
6️⃣ Tag relevant people in discussions
...
function generateNegotiationTacticsContent(
  ...
  if (language === "hindi") {
    return `🤝 प्रोफेशनल नेगोसिएशन ट��क्टिक्स & रेट ार्ड गाइड
...
🎯 TIMING STRATEGY:
• Month के beginning ���ें rates negotiate करें
...
यह किट मास्टर सर्विस एग्रीमेंट (MSA) + स्टेटमेंट ऑफ़ वर्क (SOW) का साफ़ फ़ॉर्मेट देती है, ताकि रेट, स्कोप और डेडलाइन्स बिल्कुल क्लियर हों ��र पेमेंट समय पर मिले।
...
• अप्रत्यक्ष/परिणामी क्षति के लिए क��ई दायित्व नहीं

7. कानून व विवाद समाधान
...
• Teachable ($39/month) - ऑनलाइन कोर���स
...
📝 बैकअप और सिक्योरिटी:
• Google Drive (FREE/Paid) - क्���उड स्टोरेज
...
export async function downloadFile(
  ...
  // Fallback: plain text download to guarantee delivery
  try {
    const blob = new Blob([content], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    const trimmedName = fileName.trim().replace(/\.pdf$/i, "");
    anchor.download = `${trimmedName}.txt`;
    document.body.appendChild(anchor);
    anchor.click();
    document.body.removeChild(anchor);
    URL.revokeObjectURL(url);
  } catch (e) {
    console.error("Fallback text download also failed", e);
